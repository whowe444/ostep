...
public:
    ...
    // Copy Ctor
    ConcurrentRedBlackTree(const ConcurrentRedBlackTree& other) : ConcurrentRedBlackTree() {
        Node<std::pair<K, V>>* ptr = other.root;
        this->CopyHelper(ptr);
    }

    // Copy Assignment
    ConcurrentRedBlackTree& operator=(const ConcurrentRedBlackTree& other) {
    ...
    // Move Ctor
    ConcurrentRedBlackTree(ConcurrentRedBlackTree&& other) noexcept
        :
            root(other.root),
            size(other.size)
    {
    ...
    // Move Assignment
    ConcurrentRedBlackTree& operator=(ConcurrentRedBlackTree&& other) noexcept {
        // Self assignment guard
        if (this == &other) return *this;
